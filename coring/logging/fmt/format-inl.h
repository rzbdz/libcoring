...
FMT_INLINE FMT_CONSTEXPR20 digits::result grisu_gen_digits(fp value, uint64_t error, int &exp,
  ...
  do {
    ...
    auto result = handler.on_digit(static_cast<char>('0' + digit), impl_data::power_of_10_64[exp] << -one.e, remainder,
                                   error, true);
...
struct cache_accessor<double> {
  ...
  static uint128_wrapper get_cached_power(int k) FMT_NOEXCEPT {
    ...
    uint32_t error = (pow10_recovery_errors[error_idx] >> ((k - float_info<double>::min_k) % 16) * 2) & 0x3;
