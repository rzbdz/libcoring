...
class buffer {
 ...
 public:
  ...
  const char *front() const {
    LOG_DEBUG("front, return const cannot be modifyied");
  ...
  void pop_front(size_t len) {
    assert(len <= readable());
    if (len == readable()) {
      LOG_DEBUG("pop_front %lu, but is larger, clear all", len);
      clear();
    } else {
      index_read_ += len;
      LOG_DEBUG("pop_front %lu, now: con: %lu, pro: %lu", len, index_read_, index_write_);
    }
  }

  template <typename IntType>
  IntType pop_int() {
    LOG_DEBUG("ask pop %lu bytes", sizeof(IntType));
  ...
  void push_back(size_t len) {
    LOG_DEBUG("increase length by %lu, ori:", len);
    LOG_B();
    index_write_ += len;
    LOG_DEBUG("now:");
    LOG_B();
  ...
  char *back() {
    LOG_DEBUG("back, dangerous, %lu", index_write_);
  ...
  void push_back(const void *src, size_t len) {
    LOG_DEBUG("in push_back: ask for %lu, first byte in src %d", len, (int)(*reinterpret_cast<const char *>(src)));
    char *dst = back(len);
    LOG_DEBUG("back: 0x%lx", (size_t)dst);
    ::memcpy(dst, src, len);
    LOG_DEBUG("after copy, first byte in dst: %d", (int)(*dst));
    push_back(len);
  }
  ...
  void push_back_int(IntType to_put) {
    LOG_DEBUG("push_back a int %lu: %lu bytes", (size_t)to_put, sizeof(to_put));
    to_put = coring::host_to_network(to_put);
    LOG_DEBUG("to put is now : %lu", (size_t)to_put);
    push_back(&to_put, sizeof(IntType));
  }

  std::string pop_string(size_t len) {
    LOG_DEBUG("pop_front a string: %lu", len);
    if (len > readable()) {
      LOG_DEBUG("ask for too long!");
      len = readable();
    }
